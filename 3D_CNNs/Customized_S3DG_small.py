...
class Mixed_3c(nn.Module):
    def __init__(self):
        ...
        self.branch2 = nn.Sequential(
            BasicConv3d(64, 8, kernel_size=1, stride=1),
            STConv3d(8, 16, kernel_size=3, stride=1, padding=1),
        )
        self.branch3 = nn.Sequential(
            nn.MaxPool3d(kernel_size=(3, 3, 3), stride=1, padding=1),
            BasicConv3d(64, 16, kernel_size=1, stride=1),
...
class Mixed_4b(nn.Module):
    def __init__(self):
        ...
        self.branch1 = nn.Sequential(
            ...
            STConv3d(16, 40, kernel_size=3, stride=1, padding=1),
        )
        self.branch2 = nn.Sequential(
...
class S3DG(nn.Module):
    ...
    def __init__(self, embed_size = 512, device = None, dropout = 0.5, input_channel=3, spatial_squeeze=True):
        ...
        self.features = nn.Sequential(
            ...
            Mixed_4b(),                                                                # (256, 56, 7, 6)
            Mixed_4c(),                                                                # (128, 56, 14, 12)
            nn.Dropout3d(p=0.5, inplace=True),                                         # (128, 56, 7, 6)
        )

        self.fc_out_1 = nn.Linear(128*7*6, 1660)
    ...
    def forward(self, x):
        ...
        out = self.relu(self.fc_out_2(self.dropout(out)))
