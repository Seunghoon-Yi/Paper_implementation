...
class TransformerBlock(nn.Module):
    def __init__(self, d_model, n_head, dropout = 0.4, forward_expansion = 2):
...
class Encoder(nn.Module):
    ...
    def forward(self, X, Mask):
        ...
        embeddings = self.WE(X.to(self.device))
        out = self.dropout(embeddings + self.PE(Position))              # 같은 device에 올려야..
...
class Decoder(nn.Module):
    ...
    def forward(self, X, enc_out, source_mask, target_mask):
        #X = X.to(self.device)                               # 마찬가지로 gpu 위에 올려둬야
        BS, seq_len = X.shape
